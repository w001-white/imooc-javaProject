...
if __name__ == '__main__':
    ...
    all_labels = ['鱼','肉', '素菜', '菌类', '海产品', '稀有', '辣度',  '好评', '推荐']
    labels_num = len(all_labels)

    df1 = pd.read_csv('./好评度.csv',sep=',',encoding='gbk',header='infer',error_bad_lines=False)
    (m1 , n1) = df1.shape
    data_array1 = np.array(df1.iloc[:m1+1 , 1:])
    items_users_saw_names1 = df1.columns[1:].tolist()

    df2 = pd.read_csv('./用户评分.csv',sep=',',encoding='gbk',header='infer',error_bad_lines=False)
    (m2 , n2) = df2.shape
    ...
    df3 = pd.read_csv('./菜单.csv',sep=',',encoding='gbk',header='infer',error_bad_lines=False)
    (m3 , n3) = df3.shape
